...
<style>
  ...
  #controls, #About { width:280px; padding:16px; box-sizing:border-box; overflow-y:auto; background:#f4f4f4; }
  #view { flex:1; }
  fieldset { margin-top:16px; padding:10px; }
  label, select, input, button { display:block; width:100%; margin-top:6px; font-size:14px; box-sizing:border-box; }
  button { padding:6px; margin-top:10px; cursor:pointer; }
...
<div class="container">
  ...
  <div id="About">
    <h3>About</h3>
    <p>This lattice viewer shows atoms as spheres. Adjust unit cell parameters or select a mineral to explore crystal structures. Metals use metallic shading.</p>
  </div>
</div>
...
const mineralData = {
  ...
  ruby:      {a:4.758,b:4.758,c:12.991,alpha:90,beta:90,gamma:120,bravais:"P_hexagonal",color:0xe0115f,metal:false},
  sapphire:  {a:4.758,b:4.758,c:12.991,alpha:90,beta:90,gamma:120,bravais:"P_hexagonal",color:0x0f52ba,metal:false},
  emerald:   {a:9.21,b:9.21,c:9.19,alpha:90,beta:90,gamma:120,bravais:"P_hexagonal",color:0x50c878,metal:false},
...
function buildLattice(params){
  ...
  for(let i=0;i<nx;i++) for(let j=0;j<ny;j++) for(let k=0;k<nz;k++){
    ...
    basis.forEach(bfrac=>{
      const pos=new THREE.Vector3().copy(cellOrigin)
                  .addScaledVector(aVec,bfrac[0])
                  .addScaledVector(bVec,bfrac[1])
                  .addScaledVector(cVec,bfrac[2]);
      positions.push(pos);
